...
public func lessThanOrEqual<Axis, Anchor>(_ keyPath: KeyPath<UIView, Anchor>,
	...
	return { child, parent in
		child[keyPath: keyPath].constraint(lessThanOrEqualTo: parent[keyPath: to], constant: constant)
...
public func lessThanOrEqual<Axis, Anchor>(_ keyPath: KeyPath<UIView, Anchor>,
										  ...
										  priority: UILayoutPriority = .required) -> Constraint where Anchor: NSLayoutAnchor<Axis> {
	return { child, _ in
		child[keyPath: keyPath].constraint(lessThanOrEqualTo: view[keyPath: to], constant: constant)
			.with(priority: priority)
	}
}
...
public func lessThanOrEqual<Axis, Anchor>(_ keyPath: KeyPath<UIView, Anchor>,
										  ...
										  constant: CGFloat = 0,
										  priority: UILayoutPriority = .required) -> Constraint where Anchor: NSLayoutAnchor<Axis> {
	return lessThanOrEqual(keyPath, to: view, keyPath, constant: constant, priority: priority)
}
...
///   - priority: приоритет
/// - Returns: функция Constraint
public func lessThanOrEqual<Dimension>(_ keyPath: KeyPath<UIView, Dimension>,
									   constant: CGFloat = 0,
									   priority: UILayoutPriority = .required) -> Constraint where Dimension: NSLayoutDimension {
	return { child, _ in
		child[keyPath: keyPath].constraint(lessThanOrEqualToConstant: constant)
			.with(priority: priority)
	}
}
...
public func lessThanOrEqual<Dimension>(_ keyPath: KeyPath<UIView, Dimension>,
									   ...
									   priority: UILayoutPriority = .required) -> Constraint where Dimension: NSLayoutDimension {
	return { child, parent in
		child[keyPath: keyPath].constraint(lessThanOrEqualTo: parent[keyPath: to],
...
public func lessThanOrEqual<Dimension>(_ keyPath: KeyPath<UIView, Dimension>,
									   ...
									   constant: CGFloat = 0,
									   priority: UILayoutPriority = .required) -> Constraint where Dimension: NSLayoutDimension {
	return lessThanOrEqual(keyPath, keyPath, multiplier: multiplier, constant: constant, priority: priority)
}
...
public func lessThanOrEqual<Dimension>(_ keyPath: KeyPath<UIView, Dimension>,
									   ...
									   priority: UILayoutPriority = .required) -> Constraint where Dimension: NSLayoutDimension {
	return { child, _ in
		child[keyPath: keyPath].constraint(lessThanOrEqualTo: view[keyPath: to],
										   multiplier: multiplier,
										   constant: constant)
			.with(priority: priority)
	}
...
public func lessThanOrEqual<Dimension>(_ keyPath: KeyPath<UIView, Dimension>,
									   ...
									   constant: CGFloat = 0,
									   priority: UILayoutPriority = .required) -> Constraint where Dimension: NSLayoutDimension {
	return lessThanOrEqual(keyPath, to: view, keyPath, multiplier: multiplier, constant: constant, priority: priority)
}
